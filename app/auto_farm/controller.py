...
class AutoFarmController:
    ...
    def __init__(self, connection, profile):
        ...
        self.last_lie_check_time = 0.0
        self.last_boss_check_time = 0.0
        self.last_death_check_time = 0.0
        self.last_rune_check_time = 0.0
    ...
    def _get_servo_index_for_key(self, logical_key):
        """Maps logical key name to physical Arduino servo index (0-7)."""
        logical_lower = logical_key.lower()
        actual_key = logical_lower
        if logical_lower.startswith("puzzle_"):
            actual_key = logical_lower[7:]  # "puzzle_left" -> "left"
        elif logical_lower == "interact":
            actual_key = self.settings.get("interact_key", "space").lower()
        elif logical_lower == "blink":
            actual_key = self.settings.get("blink_key", "shift").lower()
        elif logical_lower == "jump":
            actual_key = self.settings.get("jump_key", "alt").lower()
        ...
        if actual_key == "left":
            ...
            return 4
        elif actual_key in ["shift", "lshift", "rshift"]:
            return 5
        elif actual_key in ["alt", "lalt", "ralt"]:
            return 6
    ...
    def _press_key(self, logical_key, down_time=0.35, up_time=0.65, n=1):
        ...
        # Check if the action should be routed through hardware servos if enabled
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            ...
            if self.settings.get("enabled_lie_detector", False):
                # Run the CPU-heavy template matching once every 1.0 second
                if now - self.last_lie_check_time >= 1.0:
                    ...
                    # Pindai seluruh layar karena Lie Detector dapat muncul di mana saja
                    ...
                    for idx, (filename, scaled_list_gray, scaled_list_bgr, threshold_gray, threshold_bgr) in enumerate(self.pre_scaled_lie_templates):
                        ...
                        for s_idx, (s, scaled_tpl_gray) in enumerate(scaled_list_gray):
                            # Skip jika template lebih besar dari wilayah pencarian
                            if scaled_tpl_gray.shape[0] > search_frame.shape[0] or scaled_tpl_gray.shape[1] > search_frame.shape[1]:
                                continue

                            try:
                                res = cv2.matchTemplate(search_frame, scaled_tpl_gray, cv2.TM_CCOEFF_NORMED)
                        ...
                        if max_score_gray >= threshold_gray:
                            ...
                            # Koordinat absolut layar (karena search_frame adalah seluruh layar)
            ...
            if self.settings["enabled_boss"] and (now - self.last_boss_check_time >= 2.0):
                self.last_boss_check_time = now
            ...
            # 2. Death Detection
            if now - self.last_death_check_time >= 2.0:
                self.last_death_check_time = now
                gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            ...
            if self.settings["enabled_rune"] and not self.solving_rune and self.capture.calibrated and (now - self.last_rune_check_time >= 2.0):
                self.last_rune_check_time = now
